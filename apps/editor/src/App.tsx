...
import type { AdapterCapability, Asset, CameraContinuityMode, Element, ElementKind, GenerationJob, GenerationQuote, KeyframeGenerationQuote, KeyframeReference, KeyframeReferencePreview, PlannerCapability, ReferenceSelectionMode, Shot, VisualKeyframe } from "./types";
...
const CAMERA_LABELS: Record<CameraContinuityMode, string> = { free: "自由构图", prefer: "尽量保持", lock: "严格要求", directed: "指定机位变化" };
...
const compileGroupPrompt = (shot: Shot, group: Shot["interactionGroups"][number]) => {
  ...
  const cameraText = group.cameraMode==="free" ? "" : group.cameraMode==="lock" ? "严格保持机位、视角和画面裁切，不得推近、拉远、平移、摇镜或重新构图。" : group.cameraMode==="directed" ? (group.cameraInstruction?.trim() ? `只执行机位变化：${group.cameraInstruction.trim()}，其余机位属性尽量保持连续。` : "尽量保持机位。") : "尽量保持机位。";
  return `动作意图：${group.instruction}。按相对时间推进：${states.join("；")}。保持人物身份、左右手、朝向、物体归属、接触/支撑关系和背景连续。${cameraText}`;
};
...
export function App() {
  ...
  const [referenceMode,setReferenceMode]=useState<ReferenceSelectionMode>("auto");
  const [manualReferenceIds,setManualReferenceIds]=useState<string[]>([]);
  const [cameraMode,setCameraMode]=useState<CameraContinuityMode>("prefer");
  const [cameraInstruction,setCameraInstruction]=useState("");
  const [referencePreview,setReferencePreview]=useState<KeyframeReferencePreview|null>(null);
  ...
  useEffect(()=>{setManualReferenceIds([])},[activeMaskKeyframe?.id]);
  useEffect(()=>{const target=activeInteractionGroup??primary;const keyframe=activeInteractionKeyframe??activeKeyframe;if(!shot||!target||!keyframe){setReferencePreview(null);return;}let cancelled=false;const fetchPreview=async()=>{try{const response=await fetch(`${API}/keyframe-generations/references`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({shotId:shot.id,targetType:activeInteractionGroup?"interactionGroup":"element",targetId:target.id,keyframeId:keyframe.id,referenceMode,manualReferenceIds})});if(cancelled)return;setReferencePreview(response.ok?await response.json():null)}catch{if(!cancelled)setReferencePreview(null)}};void fetchPreview();return()=>{cancelled=true}},[shot?.id,activeMaskKeyframe?.id,activeInteractionGroup?.id,primary?.id,referenceMode,manualReferenceIds,lastAcceptedGeneration]);
  ...
  const generateActionGroup = async (anchorKeyframeId: string, memberIds:string[] = []) => {
    ...
    if(cameraMode==="directed"&&!cameraInstruction.trim()){setStatus("指定机位变化需要填写机位指令");return;}
    setPlanning(true); setStatus("正在生成动作/互动组…");
    try {
      await saveShot(shot);
      const response = await fetch(`${API}/action-groups`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ shotId: shot.id, anchorKeyframeId, memberIds, actionIntent:intent, planner: planPlanner, embellishment, desiredDurationFrames:actionDurationFrames, cameraMode, cameraInstruction:cameraMode==="directed"?cameraInstruction.trim():null }) });
  ...
  const generateKeyframe = async () => {
    ...
    if (cameraMode==="directed"&&!cameraInstruction.trim()) { setStatus("指定机位变化需要填写机位指令"); return; }
    const directions=referencePreview?.directions??[];
    if (manualReferenceIds.length===0&&referenceMode==="next"&&!directions.includes("after")) { setStatus("选择“后一状态”参考，但目标状态之后没有已确认图片"); return; }
    if (manualReferenceIds.length===0&&referenceMode==="previous"&&!directions.includes("before")) { setStatus("选择“前一状态”参考，但目标状态之前没有已确认图片"); return; }
    if (manualReferenceIds.length===0&&referenceMode==="both"&&(!directions.includes("before")||!directions.includes("after"))) { setStatus("选择“前后状态”参考需要目标状态前后各有一张已确认图片"); return; }
    try {
      setStatus("正在整理关键帧上下文…"); await saveShot(shot);
      const references:KeyframeReference[]=[{frame:activeInteractionKeyframe.frame,relation:"same",purpose:"scene",image:await renderFrame(activeInteractionKeyframe.frame,false)}];
      for (const memberId of activeInteractionGroup.members) { const member=shot.elements.find((item)=>item.id===memberId); const src=member&&(assetMap.get(member.assetId)?.url || imageAtFrame(member,activeInteractionKeyframe.frame)); if(src) references.push({relation:"timeless",purpose:"identity",image:await imageAsDataUrl(src),targetId:memberId}); }
      const payload={shotId:shot.id,targetType:"interactionGroup",targetId:activeInteractionGroup.id,keyframeId:activeInteractionKeyframe.id,instruction,references,referenceMode,manualReferenceIds,cameraMode,cameraInstruction:cameraMode==="directed"?cameraInstruction.trim():null,candidateCount:2,promptExtend:false,adapter:imageAdapter};
      if(activeInteractionKeyframe.renderPolicy==="required"){
        const quoteResponse=await fetch(`${API}/keyframe-generations/quote`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(payload)});
        if(!quoteResponse.ok)throw new Error(await quoteResponse.text());
        const quote:KeyframeGenerationQuote=await quoteResponse.json();
        const refDirectionLabel=quote.references.map((ref)=>ref.relation==="before"?`前一状态（第 ${ref.frame} 帧）`:ref.relation==="after"?`后一状态（第 ${ref.frame} 帧）`:ref.purpose==="scene"?"当前场景":"身份参考").join("、")||"无连续性参考";
        if(!window.confirm(`目标帧：第 ${activeInteractionKeyframe.frame} 帧\n连续性参考：${refDirectionLabel}\n机位模式：${CAMERA_LABELS[cameraMode]}${cameraMode==="directed"?`（${cameraInstruction.trim()}）`:""}\n模型：${imageAdapter}\n候选数量：${quote.candidateCount}\n预计费用：供应商未提供估价\n\n最终提示词：\n${quote.finalInstruction}\n\n尾帧图片会产生模型费用。确认生成候选？生成后仍需由你选择并采用，不会自动扣费生成视频。`)){setStatus("已取消尾帧图片生成");return;}
      }
      const response = await fetch(`${API}/keyframe-generations`, { method:"POST", headers:{"Content-Type":"application/json"}, body:JSON.stringify(payload) });
      if (!response.ok) throw new Error(await response.text());
  ...
  const generateElementKeyframe = async () => {
    ...
    try{
      ...
      const references:KeyframeReference[]=[{frame:activeKeyframe.frame,relation:"same",purpose:"scene",image:await renderFrame(activeKeyframe.frame,false)}]; const assetImage=assetMap.get(primary.assetId)?.url; if(assetImage)references.push({relation:"timeless",purpose:"identity",image:await imageAsDataUrl(assetImage),targetId:primary.id});
      const response=await fetch(`${API}/keyframe-generations`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({shotId:shot.id,targetType:"element",targetId:primary.id,keyframeId:activeKeyframe.id,instruction,references,referenceMode,manualReferenceIds,cameraMode,cameraInstruction:cameraMode==="directed"?cameraInstruction.trim():null,candidateCount:2,promptExtend:false,adapter:imageAdapter})});
  ...
  return <main className="app-shell">
    ...
    <section className="workspace">
      ...
      <section className="stage-column">
        ...
        <div className="inspector">
          ...
          {activeKeyframe && <><textarea className="state-description" aria-label="元素关键帧状态描述" placeholder="只描述当前画面可见状态，例如：女孩背对镜头，双手自然下垂" value={activeKeyframe.instruction??""} onChange={(e)=>updateKeyframe(primary.id,activeKeyframe.id,{instruction:e.target.value,locked:false})}/>{activeKeyframe.instruction?.trim()&&<div className="action-group-actions"><label>动作意图<input value={actionIntent} onChange={(event)=>setActionIntent(event.target.value)} placeholder="例如：转身面向镜头并微笑"/></label><label>动作时长（帧）<input type="number" min="1" max={Math.max(1,shot.durationFrames-activeKeyframe.frame)} value={actionDurationFrames} onChange={(event)=>setActionDurationFrames(Math.max(1,+event.target.value||48))}/><small>普通动作建议约 48 帧 / 2 秒；复杂动作可调长</small></label><label>规划机位<select aria-label="规划机位连续性" value={cameraMode} onChange={(event)=>setCameraMode(event.target.value as CameraContinuityMode)}><option value="free">自由构图</option><option value="prefer">尽量保持</option><option value="lock">严格要求</option><option value="directed">指定机位变化</option></select>{cameraMode==="directed"&&<input aria-label="规划机位变化指令" placeholder="例如：缓慢推近" value={cameraInstruction} onChange={(event)=>setCameraInstruction(event.target.value)}/>}</label><button disabled={planning||!actionIntent.trim()} onClick={()=>generateActionGroup(activeKeyframe.id)}>{planning?<LoaderCircle className="spinning" size={14}/>:<Wand2 size={14}/>}生成动作组</button><details className="interaction-members"><summary>互动对象 · {interactionTargetIds.length} 个</summary>{shot.elements.filter((element)=>element.id!==primary.id&&element.kind!=="background"&&element.kind!=="foreground").map((element)=><label key={element.id}><input type="checkbox" checked={interactionTargetIds.includes(element.id)} onChange={()=>setInteractionTargetIds((current)=>current.includes(element.id)?current.filter((id)=>id!==element.id):[...current,element.id])}/>{element.name||element.id}</label>)}</details><button disabled={planning||interactionTargetIds.length===0||!actionIntent.trim()} onClick={()=>generateActionGroup(activeKeyframe.id,interactionTargetIds)}><Wand2 size={14}/>生成互动组</button><label className="embellishment">修饰<input type="range" min="0" max="1" step="0.1" value={embellishment} onChange={(event)=>setEmbellishment(+event.target.value)}/><span>{Math.round(embellishment*100)}%</span></label></div>}<div className="interaction-director-actions"><span>关键帧描述当前状态；动作意图描述接下来发生什么。</span><div className="keyframe-ref-controls"><label>连续性参考<select aria-label="元素连续性参考" value={referenceMode} onChange={(event)=>setReferenceMode(event.target.value as ReferenceSelectionMode)}><option value="auto">自动</option><option value="none">不使用</option><option value="previous">前一状态</option><option value="next">后一状态</option><option value="both">前后状态</option></select></label><label>机位<select aria-label="元素机位连续性" value={cameraMode} onChange={(event)=>setCameraMode(event.target.value as CameraContinuityMode)}><option value="free">自由构图</option><option value="prefer">尽量保持</option><option value="lock">严格要求</option><option value="directed">指定机位变化</option></select></label>{cameraMode==="directed"&&<input aria-label="元素机位变化指令" placeholder="例如：缓慢推近" value={cameraInstruction} onChange={(event)=>setCameraInstruction(event.target.value)}/>}</div>{referencePreview&&referencePreview.references.length>0&&<div className="reference-thumbs">{referencePreview.references.map((reference,index)=><div className="ref-thumb" key={`${reference.relation}:${reference.frame}:${index}`}><img src={reference.image} alt={reference.relation}/><span>{reference.relation==="before"?"目标之前":reference.relation==="after"?"目标之后":"同帧"} · 第 {reference.frame} 帧 · 已确认</span></div>)}</div>}{referencePreview&&referencePreview.candidates.length>0&&<div className="reference-thumbs manual"><div className="manual-label">手动指定参考状态（覆盖自动模式）</div>{referencePreview.candidates.map((candidate)=>{const selected=manualReferenceIds.includes(candidate.keyframeId);return <button type="button" className={`ref-thumb ${selected?"selected":""}`} key={candidate.keyframeId} onClick={()=>setManualReferenceIds((current)=>current.includes(candidate.keyframeId)?current.filter((id)=>id!==candidate.keyframeId):[...current,candidate.keyframeId])}><img src={candidate.image} alt={candidate.relation}/><span>{candidate.relation==="before"?"目标之前":"目标之后"} · 第 {candidate.frame} 帧 · {selected?"已选中":"点击选择"}</span></button>})}</div>}{manualReferenceIds.length>0&&<div className="ref-warning">已手动选择 {manualReferenceIds.length} 个参考状态，将覆盖自动选择。</div>}{referencePreview&&referencePreview.warnings.length>0&&<div className="ref-warning">{referencePreview.warnings[0]}</div>}<div className="element-generate-actions"><button className="generate-button" disabled={activeKeyframeJob?.status==="queued"||activeKeyframeJob?.status==="running"} onClick={generateElementKeyframe}>{activeKeyframeJob?.status==="queued"||activeKeyframeJob?.status==="running"?<LoaderCircle className="spinning" size={14}/>:<Sparkles size={14}/>}生成两个候选</button><button onClick={generateElementTransition}><Film size={14}/>生成到下一关键帧</button></div></div>
    ...
    {activeInteractionGroup && activeInteractionKeyframe && <section className={`interaction-director-panel ${activeInteractionGroup.kind}`}>
      ...
      <div className="keyframe-ref-controls"><label>连续性参考<select aria-label="连续性参考" value={referenceMode} onChange={(event)=>setReferenceMode(event.target.value as ReferenceSelectionMode)}><option value="auto">自动</option><option value="none">不使用</option><option value="previous">前一状态</option><option value="next">后一状态</option><option value="both">前后状态</option></select></label><label>机位<select aria-label="机位连续性" value={cameraMode} onChange={(event)=>setCameraMode(event.target.value as CameraContinuityMode)}><option value="free">自由构图</option><option value="prefer">尽量保持</option><option value="lock">严格要求</option><option value="directed">指定机位变化</option></select></label>{cameraMode==="directed"&&<input aria-label="机位变化指令" placeholder="例如：缓慢推近" value={cameraInstruction} onChange={(event)=>setCameraInstruction(event.target.value)}/>}</div>
      {referencePreview&&referencePreview.references.length>0&&<div className="reference-thumbs">{referencePreview.references.map((reference,index)=><div className="ref-thumb" key={`${reference.relation}:${reference.frame}:${index}`}><img src={reference.image} alt={reference.relation}/><span>{reference.relation==="before"?"目标之前":reference.relation==="after"?"目标之后":"同帧"} · 第 {reference.frame} 帧 · 已确认</span></div>)}</div>}
      {referencePreview&&referencePreview.candidates.length>0&&<div className="reference-thumbs manual"><div className="manual-label">手动指定参考状态（覆盖自动模式）</div>{referencePreview.candidates.map((candidate)=>{const selected=manualReferenceIds.includes(candidate.keyframeId);return <button type="button" className={`ref-thumb ${selected?"selected":""}`} key={candidate.keyframeId} onClick={()=>setManualReferenceIds((current)=>current.includes(candidate.keyframeId)?current.filter((id)=>id!==candidate.keyframeId):[...current,candidate.keyframeId])}><img src={candidate.image} alt={candidate.relation}/><span>{candidate.relation==="before"?"目标之前":"目标之后"} · 第 {candidate.frame} 帧 · {selected?"已选中":"点击选择"}</span></button>})}</div>}
      {manualReferenceIds.length>0&&<div className="ref-warning">已手动选择 {manualReferenceIds.length} 个参考状态，将覆盖「{referenceMode==="auto"?"自动":referenceMode==="none"?"不使用":referenceMode==="previous"?"前一状态":referenceMode==="next"?"后一状态":"前后状态"}」模式。</div>}
      {referencePreview&&referencePreview.warnings.length>0&&<div className="ref-warning">{referencePreview.warnings[0]}</div>}
      <div className="interaction-director-actions"><span>系统自动附带完整场景与成员身份参考；连续性参考与机位由你选择。</span><div className="element-generate-actions"><button className="generate-button" disabled={activeKeyframeJob?.status==="queued"||activeKeyframeJob?.status==="running"} onClick={generateKeyframe}>{activeKeyframeJob?.status==="queued"||activeKeyframeJob?.status==="running"?<LoaderCircle className="spinning" size={14}/>:<Sparkles size={14}/>} {activeInteractionKeyframe.renderPolicy==="required"?"生成尾帧候选":"生成两个候选"}</button><button onClick={generateInteractionToNext}><Film size={14}/>生成到下一关键帧</button></div></div>
